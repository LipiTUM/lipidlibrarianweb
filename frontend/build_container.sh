...
ctr_lipidlibrarianweb_frontend=$(buildah from docker.io/library/nginx:mainline-alpine)

# set environment variables
buildah config --env BACKEND_HOSTNAME=${BACKEND_HOSTNAME} $ctr_lipidlibrarianweb_frontend
buildah config --env BACKEND_PORT=${BACKEND_PORT} $ctr_lipidlibrarianweb_frontend

# copy compiled angular app from ctr_angular_build
buildah copy --from $ctr_angular_build $ctr_lipidlibrarianweb_frontend /app/dist/lipidlibrarianweb /usr/share/nginx/html

# prepare nginx configuration
buildah copy $ctr_lipidlibrarianweb_frontend ./nginx/default.conf.template /etc/nginx/templates/

buildah commit --rm $ctr_lipidlibrarianweb_frontend lipidlibrarianweb_frontend:latest
